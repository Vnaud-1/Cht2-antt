...
from modules import port_scanner, service_detector, banner_grabber, network_mapper, vuln_checker, email_sender
import asyncio, os
...
def scan():
    ...
    # Thực hiện từng module theo chế độ
    if mode in ['scan', 'all']:
    ...
    if mode in ['banner', 'all']:
        result['banner'] = {p: banner_grabber.grab_banner(target, p) for p in ports}
    if mode in ['map', 'all']:
        result['map'] = network_mapper.map_network()
    if mode in ['vuln', 'all']:
        result['vuln'] = vuln_checker.check_vulns(ports)

    # Tạo nội dung email kết quả
